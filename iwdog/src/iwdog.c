...
#define OVYL_IWDOG_THREAD_PRIORITY K_PRIO_PREEMPT(CONFIG_OVYL_IWDOG_THREAD_PRIORITY)
#define OVYL_IWDOG_THREAD_STACK_SIZE CONFIG_OVYL_IWDOG_THREAD_STACK_SIZE

/* Ensure feed interval is less than timeout */
BUILD_ASSERT(CONFIG_OVYL_WATCHDOG_FEED_INTERVAL_MS < CONFIG_OVYL_WATCHDOG_TIMEOUT_MS,
             "Watchdog feed interval must be less than watchdog timeout");

#if defined(CONFIG_OVYL_IWDOG_LOG_PANIC_ON_WARNING)
BUILD_ASSERT(CONFIG_OVYL_IWDOG_LOG_PANIC_THRESHOLD_MS > 0 &&
                 CONFIG_OVYL_IWDOG_LOG_PANIC_THRESHOLD_MS < CONFIG_OVYL_WATCHDOG_TIMEOUT_MS,
             "OVYL_IWDOG_LOG_PANIC_THRESHOLD_MS must be in (0, TIMEOUT)");
#endif
...
#endif
#ifdef CONFIG_OVYL_IWDOG_LOG_PANIC_ON_WARNING
    struct k_timer panic_timer; /* Timer for LOG_PANIC before reset */
    atomic_t did_panic;         /* Ensure LOG_PANIC called only once */
#endif
} prv_inst;

static K_THREAD_STACK_DEFINE(prv_thread_stack, OVYL_IWDOG_THREAD_STACK_SIZE);
...
static void prv_warning_work_handler(struct k_work *work) {
    ...
    struct ovyl_iwdog_warning_event warning_evt = {.time_until_reset_ms =
 ...
 *
 * This timer fires when CONFIG_OVYL_IWDOG_WARNING_PCT of the iwdog timeout period
...
static void prv_warning_timer_callback(struct k_timer *timer) {
    ...
    int32_t time_until_reset = (int32_t)CONFIG_OVYL_WATCHDOG_TIMEOUT_MS - (int32_t)time_since_feed;
...
static void prv_thread(void *p1, void *p2, void *p3) {
    ...
    while (1) {
        ...
        k_sleep(K_MSEC(CONFIG_OVYL_WATCHDOG_FEED_INTERVAL_MS));
    }
}
...
int ovyl_iwdog_init(void) {
    ...
    wdt_config.window.max = CONFIG_OVYL_WATCHDOG_TIMEOUT_MS;
    ...
    uint32_t warning_timeout =
        (CONFIG_OVYL_WATCHDOG_TIMEOUT_MS * CONFIG_OVYL_IWDOG_WARNING_PCT) / 100;
    ...
    uint32_t panic_timeout =
        CONFIG_OVYL_WATCHDOG_TIMEOUT_MS - CONFIG_OVYL_IWDOG_LOG_PANIC_THRESHOLD_MS;
 ...
 */
static void prv_shell_ovyl_iwdog_enable(const struct shell *sh, size_t argc, char **argv) {
    ARG_UNUSED(argc);
    ARG_UNUSED(argv);
 ...
 * @param argv Argument values
 */
static void prv_shell_ovyl_iwdog_disable(const struct shell *sh, size_t argc, char **argv) {
    ARG_UNUSED(argc);
    ARG_UNUSED(argv);
 ...
 * @param argv Argument values
 */
static void prv_shell_ovyl_iwdog_status(const struct shell *sh, size_t argc, char **argv) {
    ARG_UNUSED(argc);
    ARG_UNUSED(argv);
    ...
    shell_print(sh, "  Timeout: %d ms", CONFIG_OVYL_WATCHDOG_TIMEOUT_MS);
    shell_print(sh, "  Feed interval: %d ms", CONFIG_OVYL_WATCHDOG_FEED_INTERVAL_MS);
}
